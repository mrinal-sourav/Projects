...
neural_net = load_model('models/ball_size_30_light_v2.h5')
...
# combines a state with all possible actions for the neural network to predict the q_value
...
def game_loop(no_of_rounds=1):
    ...
    while not stop:
        ...
        if q_ball.speed == 0 and red_ball.speed == 0.0 and black_ball.speed == 0.0:
            ...
            # given the state, generate test data and get predictions from the model
            test_data = generate_test(new_state, rads, speed_range)
            predictions = neural_net.predict(test_data)
            
            # select the best possible action from the predictions and set q_ball's angle and speed 
            q_values = np.concatenate((test_data, predictions), axis=1)
            q_values = q_values[q_values[:,8].argsort()[::-1]]
            action = q_values[0] 
