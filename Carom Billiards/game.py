...
# to keep score 
score_board = {'score': [0, 0], 'player': ['Computer', 'Human']} 
...
class Ball:
    ...
    def __init__(self, size = 20, position = (int(right/2),int(bottom/2)), color = (0,0,0), speed = 0.0, angle = 0):
        self.size =  size
        self.x = position[0]
        self.y = position[1]
        self.pos = position
...
# bfs is used to search for the closest position ball2 could take
...
def collide(b1,b2):
    ...
    # adjust overlap using bfs to find the best point without overlap
