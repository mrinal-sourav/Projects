...
class Ball:
    ...
    def __init__(self, size = 50, position = (int(right/2),int(bottom/2)), color = (0,0,0), speed = 0.0, angle = 0):
        self.size =  size
        self.x = position[0]
        self.y = position[1]
        self.color = color
...
def game_loop(no_of_rounds):
    ...
    while not stop:
        ...
        if q_ball.speed == 0 and red_ball.speed == 0.0 and black_ball.speed == 0.0:
            ...
            q_ball.angle = random.choice(rads)
            q_ball.speed = random.choice(speed_range)
            new_action = [q_ball.angle, q_ball.speed] 
            state_action = new_state + new_action
...
for i in range(3000,4000):
    ...
    np.save('training_data/train_inputs_para_50', train_inputs)
    np.save('training_data/train_targets_para_50', train_targets)
